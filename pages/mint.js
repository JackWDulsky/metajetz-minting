...
export default function Mint() {
  ...
  const [mintPrice, setSalePrice] = useState(0)
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setMaxMintAmount(
        isPublicSale ? config.maxMintAmount : config.presaleMaxMintAmount
      )
      setSalePrice(
        isPublicSale ? config.publicSalePrice : config.preSalePrice
      )
    }
  ...
  const presaleMintHandler = async () => {
    if(!isPreSale) {
      setStatus({
        success: false,
        message: "Pre-Sale Disabled"
      })
      return;
    }
    setIsMinting(true)

    const { success, status } = await presaleMint(mintAmount, mintPrice)
  ...
  const publicMintHandler = async () => {
    if(!isPublicSale) {
      setStatus({
        success: false,
        message: "Public Sale Disabled"
      })
      return;
    }
    setIsMinting(true)

    const { success, status } = await publicMint(mintAmount, mintPrice)
  ...
  return (
    <div className="h-full w-full flex flex-col items-center justify-center bg-brand-background ">
      <div className="relative w-full h-full flex flex-col items-center justify-center">
        <img
          src="/images/blur.jpeg"
          className="animate-pulse-slow absolute inset-auto block w-full object-cover"
          alt=""
          style={{height: "calc(100vh -80px)"}}
        />

        <div className="flex flex-col items-center justify-center h-full w-full px-2 md:px-10">
          <div style={{height: "calc(100vh - 80px)"}} className="relative z-1 md:max-w-3xl w-full bg-gray-900/90 filter backdrop-blur-sm rounded-md px-2 md:px-10 flex flex-col items-center">
            {wallet && (
              <button
            ...
            <h1 className="font-coiny uppercase font-bold text-3xl md:text-4xl bg-gradient-to-br  from-brand-green to-brand-blue bg-clip-text text-transparent mt-3">
              {paused ? 'Paused' : isPublicSale ? 'Public Sale' : 'Pre-Sale'}
            </h1>
            ...
            <div className="flex flex-col md:flex-row md:space-x-14 w-full mt-10 md:mt-14">
              <div className="relative w-full">
                ...
                <img
                  src="/images/Sparrow977_3.png"
                  className="object-cover w-full sm:h-[280px] md:w-[250px] rounded-md margin-auto sm:h-[25vh]"
                  style={{margin: "auto", maxHeight: "25vh"}}
                  alt=""
                />
              </div>
              ...
              <div className="flex flex-col items-center w-full px-4 md:mt-0">
                <div className="font-coiny flex items-center justify-between w-full">
                  ...
                  <span className="flex items-center justify-center flex-1 grow text-center font-bold text-brand-meta-orange text-3xl md:text-4xl" style={{margin: "auto"}}>
                    {mintAmount}
                  </span>
                ...
                {wallet ? (
                  <button
                    ...
                    onClick={isPublicSale ? publicMintHandler : presaleMintHandler}
                  >
                    {paused ? "" : isMinting ? 'Minting...' : 'Mint'}
                  </button>
                ) : (
                  <button
                    className="font-coiny mt-12 w-full bg-gradient-to-br from-brand-meta-red to-brand-meta-orange shadow-lg px-6 py-3 rounded-md text-2xl text-white hover:shadow-pink-400/50 mx-4 tracking-wide uppercase"
                    onClick={() => connect()}
                  >
                    Connect Wallet
                  </button>
            ...
            <div className="border-t border-gray-800 flex flex-col items-center mt-10 py-2 w-full">
              <h3 className="font-coiny text-2xl text-brand-meta-red uppercase mt-6">
                Contract Address
              </h3>
              <a
              ...
              >
                <span className="break-all ...">{config.contractAddress
                ? config.contractAddress.slice(0, 8) +
                  '...' +
                  config.contractAddress.slice(-8)
                : ''}</span>
              </a>
            </div>
          </div>
