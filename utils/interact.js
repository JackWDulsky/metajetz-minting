...
const contract = require('../artifacts/contracts/MetaJetzSparrow977.sol/MetaJetzSparrow977.json')
...
export const presaleMint = async (mintAmount, mintPrice) => {
  ...
  const tx = {
    ...
    value: parseInt(
      web3.utils.toWei(String(mintPrice * mintAmount), 'ether')
    ).toString(16), // hex
...
export const publicMint = async (mintAmount, mintPrice) => {
  if (!window.ethereum.selectedAddress) {
    return {
      success: false,
      status: 'To be able to mint, you need to connect your wallet'
